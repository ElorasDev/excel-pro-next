...
// Helper function to display plan name that was missing in original code
...
const Etransfer = () => {
  ...
  const finalPrice = isFirstTime ? `${parseInt(price) + 75}` : price;
  ...
  // Move the useEffect for selectedPlan initialization BEFORE any conditional returns
  useEffect(() => {
    try {
  ...
  }, [division]); // Only execute when division changes
  ...
  const createTransfer = async () => {
    ...
    try {
      // Check if userId is available from the API call
      if (!userId) {
  ...
  // Render loading state if still loading user data
  if (!userId && !error) {
    return (
      ...
      {step === 1 && (
        <div>
          ...
          <div className="bg-gray-100 p-4 rounded mb-6">
            ...
            <hr className="my-2" />
            <p className="font-bold">Selected Program Details:</p>
            <p>Plan: {getPlanDisplayName(selectedPlan)}</p>
            <p>Amount: ${finalPrice}</p>
            <p className="mt-2 text-sm text-gray-600">
              {isFirstTime
                ? "This appears to be your first time subscribing."
                : "Welcome back! You have subscribed with us before."}
            </p>
          </div>
          <p className="mb-4">
            After clicking the button below, you will see the transfer
            instructions.
            <br />
            Note: After completing the payment, you must return to this page and
      ...
      {step === 2 && (
        <div>
          ...
          <div className="bg-gray-100 p-4 rounded mb-6">
            ...
            <p className="font-bold mt-4">Amount to transfer: ${finalPrice}</p>
            <p className="mt-2">
              Please include reference number {transferId} in your transfer
              notes.
            </p>
          </div>

          <p className="mb-4">
            After you have completed the transfer, click the button below to
            confirm:
          </p>
